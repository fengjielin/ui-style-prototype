...
在通用平台原型之外，另有一套**独立的「童蹊社区-原型」原型**，位于 `ui-style-prototype/medal-system/`，与通用平台原型互不干扰（数据前缀 `demo.medal.`）。

**入口**：`medal-system/index.html`（6 张入口卡：平台管理员 / 园长 / 园长小程序 / 教师 / 教师工作台 / 评委）

## 结构

```
medal-system/
├─ index.html            角色入口（6 张入口卡，家长端不展示）
├─ assets/
...
├─ pc/
│  ├─ admin.html         PC 管理后台外壳（admin 全量菜单 / principal 仅排行榜）
│  ├─ teacher.html       教师 PC 个人工作台（我的活动 / 园内排行 / 我的勋章 / 我的奖金）
│  └─ judge.html         评委打分端（任务列表 + 打分弹窗，无侧栏）
└─ mobile/               角色化首页（hero + 勋章激励卡 + 功能宫格 + tabBar）
                          + 勋章二级页（activity / rank / medal / mine，无 tabBar）
                          + principal.html 园长小程序端（仅排行榜）
```

## 演示路径

1. 打开 `medal-system/index.html`，选角色进入对应端。
2. **平台管理员** → `pc/admin.html`：核心闭环页真实实现（活动列表状态机 / 园内排行榜 5 榜+TOP10 / 家长三色进度 / 积分方案权重配置 / 勋章门槛联动 / 勋章档案 / 月度发放清单含离职剔除 / 教师管理离职标注），其余菜单"建设中"占位。
3. **园长** → 仅排行榜（园内排行榜 / 家长进度看板）；**园长小程序** → `mobile/principal.html`（园内排行榜 / 家长进度）；**教师** → 小程序端角色化首页；**教师工作台** → `pc/teacher.html` 教师 PC 个人工作台；**评委** → `pc/judge.html` 打分并留痕；**家长** → 简版首页（无勋章卡片）。
4. 交互状态（活动流转 / 勋章 / 评委留痕 / 消息已读）存 localStorage `demo.medal.*`，刷新保留，「重置演示数据」可恢复默认。

对应需求/方案文档：`2026-08-11-01-需求-童蹊社区-原型需求拆解.md`、`2026-08-11-02-方案-童蹊系统菜单模块设计.md`。
